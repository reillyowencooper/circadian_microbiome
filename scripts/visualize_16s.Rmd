...
time_alphadiv_plot <- ggplot(data_simpson, aes(id, 1/estimate, group = id, color = id)) + 
  ...
  scale_x_discrete(labels = c("Daphnia\nmagna,\nday",
                              ...
                              "Daphnia\ndentifera,\nday",
                              "Daphnia\ndentifera,\nnight")) +
  stat_compare_means(comparisons = comparisons, label.y = c(19, 19)) +
  stat_compare_means(method = "anova", label.y = 15) +
  ylab("Inverse Simpson Index Estimate")
# Composition, family level
...
time_composition_plot <- ggplot(sp_time_melted, aes(id, Abundance, fill = Family)) +
  geom_bar(stat = "identity", position = "stack") +
...
genus_abundance_plot <- sp_time_melted %>% 
  ...
  scale_color_manual(values = id_colors, 
                     labels = c("Daphnia magna, day",
                                "Daphnia magna, night",
                                "Daphnia dentifera, day",
                                "Daphnia dentifera, night"),
                     name = "Genus & Time") +
...
figure_two <- plot_grid(plot_grid(time_alphadiv_plot, 
                    time_composition_plot, 
                    nrow = 1, rel_widths = c(0.5, 0.8), labels = "auto"),
          genus_abundance_plot,
          nrow = 2, labels = c("", "c"), rel_heights = c(.75, 1))
